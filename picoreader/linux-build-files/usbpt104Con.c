...
void CollectData()
{
	...
	while((g_status == PICO_OK || g_status == PICO_NO_SAMPLES_AVAILABLE || g_status == PICO_WARNING_REPEAT_VALUE))
	{
		...
		//for(channel = 0; channel < NUM_CHANNELS && (g_status == PICO_OK || g_status == PICO_NO_SAMPLES_AVAILABLE); channel++)
		{
			...
			//printf("ch %d, status %d, NS %d, val =%d\n", channel, g_status, PICO_NO_SAMPLES_AVAILABLE,values[channel]);
		...
		// write to file
		fprintf(outfile,"%d-%02d-%02d %02d:%02d:%02d\t\t", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
...
void main()
{
	...
	for (channel = 0; channel < NUM_CHANNELS; channel++)
	{
		channelSettings[channel].measurementType	= USBPT104_RESISTANCE_TO_10K;
	...
	int8_t unitname[25];
	g_status = UsbPt104GetUnitInfo(g_handle, unitname, infoLength, &requiredSize, (PICO_INFO) 4);
	unitname[5] = '_';
	printf("Unit Name: %s\n",unitname);
	// Display MAC Address
